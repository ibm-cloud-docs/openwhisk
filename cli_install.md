...
* Fixed a bug that results in incorrect IBM Cloud Functions API host, authentication key and namespace after region and IBM Cloud API host switch. 
* Fixed a bug with the command `ibmcloud wsk namespace delete`. The bug sometimes reports success message when a namespace space delete command fails. 
* Updated `ibmcloud wsk api` to support API create, delete, get, list in IAM-based namespaces. 
* `bx wsk namespace get --properties` now displays CRN and Service Id. 
