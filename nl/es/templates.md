...
|:-----------------|:-----------------|:-----------------|:-----------------|
| [{{site.data.keyword.cloudant_short_notm}} Events](#cloudant-template) | Cuando se edite o añada un documento en una base de datos de {{site.data.keyword.cloudantfull}}, registra el cambio en la consola. | Node.js, Swift, Python, PHP | [Código](https://github.com/ibm-functions/template-cloudant-trigger){: external} |
| [Upload Image](#image-template) | Utiliza una acción web para cargar una imagen en el grupo de una instancia de {{site.data.keyword.cos_full}} y, a continuación, recupera una miniatura de la imagen. | Node.js | [Código](https://github.com/ibm-functions/template-cloud-object-storage){: external}
| [Get HTTP Resource](#get-http-resource-template) | Invoca una acción web utilizando un suceso HTTP y obtiene datos de la API de Yahoo Weather. | Node.js, Python | [Código](https://github.com/ibm-functions/template-get-external-resource){: external}
| [Hello World](#hello-world-template) | Crea una acción básica que acepte un objeto JSON como único parámetro. | Node.js, Swift, Python, PHP | [Código](https://github.com/ibm-functions/template-hello-world){: external}
| [{{site.data.keyword.messagehub}} Events](#messagehub-events-template) | Cuando se añaden datos nuevos en un tema de {{site.data.keyword.messagehub_full}}, registra el cambio en la consola. | Node.js, Swift, Python, PHP | [Código](https://github.com/ibm-functions/template-messagehub-trigger){: external}
| [Periodic Slack Reminder](#slack-reminder-template) | Utiliza un webhook para Slack basado en un desencadenante periódico. | Node.js, Swift, Python, PHP | [Código](https://github.com/ibm-functions/template-reminder-slack){: external}

Puede acceder al código de cada plantilla en su repositorio de código, modificarla como desee y crear su propia función a partir de ella.
{: tip}
...
1. Vaya a la [página Crear](https://cloud.ibm.com/openwhisk/create){: external} en la consola de {{site.data.keyword.openwhisk_short}}.
...
5. Cree el desencadenante de {{site.data.keyword.cloudant_short_notm}}. Los desencadenantes invocan acciones cuando reciben sucesos de orígenes de sucesos. Para crear el desencadenante, debe proporcionar una instancia de servicio de {{site.data.keyword.messagehub}} y credenciales de servicio.
    ...
    5. En la lista **Instancia de Cloudant**, ahora puede seleccionar **Especificar sus propias credenciales** y proporcionar la información siguiente.
        ...
        * `Username` - Su nombre de usuario de {{site.data.keyword.cloudant_short_notm}}.
        * `Password` - Su contraseña de {{site.data.keyword.cloudant_short_notm}}.
        * `Host` - `<username>.cloudant.com`. 
        * `Database` - El nombre de su base de datos de {{site.data.keyword.cloudant_short_notm}}.

6. Pulse **Desplegar**. Una vez desplegada la plantilla, puede realizar más cambios en el código para personalizarlo según sea necesario, o retroceder y comprobar el catálogo de plantillas disponibles.
...
La plantilla Upload Image crea una acción web que puede utilizar para cargar una imagen en un grupo de {{site.data.keyword.cos_full_notm}} mediante una pequeña interfaz. A continuación, la plantilla recupera la imagen como una miniatura y la muestra en la interfaz de la acción web.
...
6. La plantilla precisa de credenciales de servicio de una instancia de servicio de {{site.data.keyword.cos_full_notm}}. En la lista de **{{site.data.keyword.cos_full_notm}}**, seleccione una de las siguientes opciones:
  * **Crear una instancia**. Si no tiene una instancia de servicio existente, seleccione esta opción para crear una.
...
3. Despliegue la plantilla, utilizando un nombre de paquete personalizado y el nombre del grupo de {{site.data.keyword.cos_full_notm}} como variables de entorno.
    ```
    PACKAGE_NAME=<name> BUCKET_NAME=<name> ibmcloud fn deploy -m manifest.yaml
    ```
    {: pre}

4. En la [página Acciones ](https://cloud.ibm.com/openwhisk/actions){: external} de la consola de {{site.data.keyword.openwhisk_short}}, pulse en la acción `process-change`.
...
La plantilla {{site.data.keyword.messagehub}} Events crea una acción y un desencadenante que activa dicha acción. El desencadenante se activa cada vez que se añade un elemento nuevo al tema de {{site.data.keyword.messagehub}} seleccionado durante la creación de la plantilla.
...
5. Cree el desencadenante de {{site.data.keyword.messagehub}}. Los desencadenantes invocan acciones cuando reciben sucesos de orígenes de sucesos. Para crear el desencadenante, debe proporcionar una {{site.data.keyword.messagehub}} instancia de servicio y credenciales de servicio.
    ...
    5. En la lista **Instancia de MessageHub**, ahora puede seleccionar **Especificar sus propias credenciales** y proporcionar la información siguiente:
        * `Username` - Su nombre de usuario de {{site.data.keyword.messagehub}}.
        * `Password` - Su contraseña de {{site.data.keyword.messagehub}}.
        * `kafka_admin_url` - Su URL REST de administrador de {{site.data.keyword.messagehub}}.
        * `Database` - El nombre de su base de datos de {{site.data.keyword.messagehub}}.
        * `Topic` - El tema al que suscribirse.
...
5. Cree el desencadenante de Periodic Slack Reminder. Los desencadenantes invocan acciones cuando reciben sucesos de orígenes de sucesos.
    1. Especifique el intervalo de tiempo de desencadenante utilizando un patrón o una expresión `Cron`.
        * `Pattern` - Seleccione horas UTC para los días de la semana, horas y minutos.
...
3. Despliegue la plantilla, utilizando las variables de entorno siguientes.
    ...
    <tr><td><code>ALARM_CRON</code></td><td>Una secuencia de cron basada en la <a href="http://crontab.org">sintaxis de crontab de UNIX</a>. Utilice 5 o menos campos con el formato <code>X X X X X</code>.</td></tr>
    <tr><td><code>PACKAGE_NAME</code></td><td>Un nombre personalizado para el paquete</td></tr>
    <tr><td><code>RULE_NAME</code></td><td>Un nombre personalizado para la regla</td></tr>
    <tr><td><code>TRIGGER_NAME</code></td><td>Un nombre personalizado para el desencadenante</td></tr>
    </tbody></table>
