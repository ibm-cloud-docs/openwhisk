...
* Para obtener más información sobre el significado de los modelos que utiliza el servicio y para describir las características de personalidad, consulte [Modelos de personalidad](/docs/services/personality-insights?topic=personality-insights-models).
* Para obtener más información sobre el significado de las preferencias de consumo, consulte [Preferencias de consumo](/docs/services/personality-insights?topic=personality-insights-preferences).

La solicitud de creación de registros está inhabilitada para el servicio {{site.data.keyword.personalityinsightsshort}}. El servicio no registra ni retiene datos de las solicitudes y respuestas, independientemente de cómo esté configurada la cabecera de solicitud `X-Watson-Learning-Opt-Out`.
{: note}
...
| --- | --- | --- | --- |
| [`personality-insights-v3`](https://www.ibm.com/watson/developercloud/personality-insights/api/v3/curl.html){: external} | Paquete | `username`, `password`, `iam_access_token`, `iam_apikey`, `iam_url`, `headers`, `headers[X-Watson-Learning-Opt-Out]`, `url` | Trabajar con el servicio {{site.data.keyword.personalityinsightsshort}} V3. |
| [`profile`](https://www.ibm.com/watson/developercloud/personality-insights/api/v3/curl.html?curl#profile){: external} | Acción | `username`, `password`, `iam_access_token`, `iam_apikey`, `iam_url`, `headers`, `headers[X-Watson-Learning-Opt-Out]`, `url`, `content`, `content_type`, `content_language`, `accept_language`, `raw_scores`, `csv_headers`, `consumption_preferences` | Obtener un perfil. |
| [`profile-as-csv`](https://www.ibm.com/watson/developercloud/personality-insights/api/v3/curl.html?curl#profile-as-csv){: external} | Acción | `username`, `password`, `iam_access_token`, `iam_apikey`, `iam_url`, `headers`, `headers[X-Watson-Learning-Opt-Out]`, `url`, `content`, `content_type`, `content_language`, `accept_language`, `raw_scores`, `csv_headers`, `consumption_preferences` | Obtener un perfil como un archivo CSV. |
...
1. [Crear una instancia de servicio de {{site.data.keyword.personalityinsightsshort}}](https://cloud.ibm.com/catalog/services/personality_insights){: external}.
...
2. Utilizando el menú desplegable de espacios de nombres. seleccione el espacio de nombres donde desea instalar el paquete. 
...
7. Una vez que se ha instalado el paquete, se le redirige a la página Acciones donde puede buscar su nuevo paquete, que se denomina `personality-insights-v3`.

8. Para utilizar las acciones del paquete `personality-insights-v3`, debe enlazar las credenciales de servicio con las acciones.
  * Para enlazar las credenciales de servicio con todas las acciones del paquete, siga los pasos 5 y 6 en las instrucciones de la CLI.
...
Las funciones de este paquete utilizan la versión actual de {{site.data.keyword.personalityinsightsshort}}, 2017-10-13. Pruebe la acción `profile`.
```
...
no devolverá nada interesante. El servicio utiliza análisis lingüístico para inferir las características de personalidad intrínsecas de las personas, incluidas las del Modelo de los cinco grandes de psicología, necesidades y valores a partir de comunicaciones digitales como, por ejemplo, correos electrónicos, mensajes de texto, tweets y publicaciones en foros. El servicio puede inferir automáticamente, desde redes sociales potencialmente ruidosas, retratos de individuos que reflejan sus características de personalidad. El servicio puede inferir las preferencias de consumo en base a los resultados de sus análisis y, a partir de contenido JSON con indicación de fecha y hora, informar de su comportamiento a lo largo del tiempo."
```
{: pre}
