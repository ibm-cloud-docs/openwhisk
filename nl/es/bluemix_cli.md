...
Para obtener más información sobre el mandato del plugin, utilice `bx plugin --help` o consulte la documentación siguiente:
...
Para obtener más información sobre el mandato del plugin, utilice `bx login --help` o revise la documentación siguiente:
...
Para obtener una lista de mandatos para el plugin de {{site.data.keyword.openwhisk_short}}, ejecute `bx wsk` sin argumentos.
...
Puede utilizar `bx iam space-create` para crear más espacios bajo su organización, como "staging" y "production". Consulte el ejemplo siguiente:
```
bx iam space-create "staging"
bx iam space-create "production"
```

Para obtener más información sobre el mandato `bx iam`, consulte la documentación siguiente: https://console.bluemix.net/docs/cli/reference/bluemix_cli/bx_cli.html#bluemix_iam_space_create
...
Si ya ha iniciado la sesión, puede ejecutar el mandato `bx target` en la CLI de {{site.data.keyword.Bluemix_notm}} para cambiar regiones, organizaciones y espacios.
...
Utilice el mandato `bx target` para cambiar regiones.  Por ejemplo, si desea cambiar a la región de Reino Unido y al espacio `staging`:
```
...
Para obtener más información sobre el mandato `target`, utilice `bx target --help` o revise la documentación siguiente:
...
Todos los mandatos `wsk`, excepto el mandato `wsk bluemix login`, que ya no es necesario, funcionan del mismo modo mediante el mandato `bx wsk`.  Todos los argumentos y las opciones de mandato son los mismos.

### Host y autenticación de API
{: #api_authentication}

En la CLI de OpenWhisk, debe configurar la clave de API de autenticación y el host de API. 
Con el plugin de CLI de {{site.data.keyword.openwhisk_short}}, no es necesario que configure explícitamente la clave de API y el host de API. Debe iniciar la sesión con `bx login` y seleccionar la región y el espacio de nombres como destino mediante el mandato `bx target`. Después, todos los mandatos posteriores empiezan por `bx wsk`. La autenticación de esta manera también evita acceder a la página web de {{site.data.keyword.Bluemix_notm}} para obtener la información específica para configurar el entorno de CLI.
...
Con el plugin de CLI de {{site.data.keyword.openwhisk}}, ya no es necesario ejecutar `wsk bluemix login`. En su lugar, utilice el mandato `bx login` para iniciar sesión en {{site.data.keyword.Bluemix_notm}}, y el plugin de {{site.data.keyword.openwhisk}} utilizará automáticamente su información de destino e inicio de sesión actual. Ahora puede gestionar las API utilizando el mandato `bx wsk api`.

### Migración de scripts de despliegue
{: #migrating_deploy_scripts}

Si tiene scripts que utilizan la CLI de OpenWhisk con el binario de `wsk`, todos los mandatos funcionan de la misma manera mediante el mandato `bx wsk`. Puede modificar los scripts para utilizar el plugin de CLI de {{site.data.keyword.Bluemix_notm}} o crear un alias o un derivador para que las ejecuciones actuales de `wsk` se conviertan en `bx wsk`. Los mandatos `bx login` y `bx target` de la CLI de {{site.data.keyword.Bluemix_notm}} funcionan de modo desatendido. Con la modalidad desatendida, puede configurar el entorno antes de ejecutar mandatos `bx wsk` para desplegar y gestionar las entidades de {{site.data.keyword.openwhisk_short}}.
...
1.0.7 (02-02-2018)
* `bx wsk api` ahora acepta parámetros de vía de acceso, como `/api/{id}`. Para obtener información, consulte [Pasarela de API](./openwhisk_apigateway.html)
* Se ha restaurado el soporte de proxy.
* Se ha eliminado `swift:3`.

1.0.6 (30-01-2018)
* Arreglo de error para el mandato `bx wsk service bind` para las acciones de dentro de un paquete.
