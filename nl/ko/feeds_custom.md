...
**Hooks**, **Polling** 및 **Connections**와 같은 세 가지 아키텍처 패턴 중 하나를 사용하여 피드를 작성할 수 있습니다.

### 후크

후크 패턴에서 피드는 다른 서비스에 의해 노출된 [웹훅](https://en.wikipedia.org/wiki/Webhook){: external}을 사용하여 설정됩니다. 이 전략에서 웹축은 트리거의 실행을 위해 URL로 직접 POST를 실행하는 외부 서비스에서 구성됩니다. 이 방법은 빈도가 낮은 피드의 구현을 위한 단연코 가장 손쉽고 가장 매력적인 옵션입니다.
...
|매개변수 |설명 |
| --- | --- |
|`lifecycleEvent` | `CREATE`, `READ`, `UPDATE`, `DELETE`, `PAUSE` 또는 `UNPAUSE` |
...
1. 피드 액션이 `whisk.system/alarms` 피드를 사용하여 특정 빈도로 주기적 트리거를 설정합니다.
...
이전의 2개의 아키텍처 선택사항은 단순하고 구현이 용이합니다. 그러나 고성능 피드를 원하는 경우, 지속적 연결과 장기 폴링 또는 유사한 기술을 사용할 수 있습니다.
...
연결 기반 아키텍처는 최고 성능 옵션이지만, 조작은 폴링 및 후크 아키텍처보다 더 많은 노동 집약적입니다.
