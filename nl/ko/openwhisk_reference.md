...
{{site.data.keyword.openwhisk_short}} 액션, 트리거 및 규칙은 네임스페이스에, 그리고 가끔은 패키지에 속합니다. 

패키지에는 조치 및 피드가 포함될 수 있습니다. 패키지에는 다른 패키지가 포함될 수 없으므로 패키지 중첩은 허용되지 않습니다. 또한 엔티티는 패키지에 포함될 필요가 없습니다. 
...
조치, 트리거, 규칙, 패키지 및 네임스페이스를 포함하여 모든 엔티티의 이름은 다음 형식을 따르는 문자 시퀀스입니다. 
...
조치 구현은 Stateless 또는 *idempotent*입니다. 시스템이 이 특성을 적용하지 않으면 조치에서 유지하는 상태를 호출 간에 사용할 수 있다는 보장이 없습니다. 

더구나 조치의 인스턴스화가 다수 존재할 수 있으며, 각각의 인스턴스화에는 자체 상태가 있습니다. 조치 호출은 이러한 인스턴스화에 디스패치될 수 있습니다. 

### 호출 입력 및 출력
{: #openwhisk_semantics_invocationio}

조치와의 양방향 입출력은 키-값 쌍의 사전입니다. 키는 문자열이며 값은 유효한 JSON 값입니다. 

### 조치의 호출 순서 지정
{: #openwhisk_ordering}

조치의 호출은 순서가 지정되어 있지 않습니다. 사용자가 REST API 또는 명령행에서 조치를 두 번 호출하는 경우, 두 번째 호출이 첫 번째 호출 이전에 실행될 수 있습니다. 조치에 부작용이 있는 경우, 이는 순서와 무관하게 관측될 수 있습니다. 

또한 조치가 자동으로 실행된다는 보장은 없습니다. 두 개의 조치가 동시에 실행될 수 있으며 해당 부작용이 개입될 수 있습니다. OpenWhisk는 부작용과 관련하여 특정 동시 일관성 모델을 보장하지 않습니다. 동시성 부작용은 구현에 따라 다릅니다. 
...
시스템은 조치를 한 번만 호출하며, 최종적으로는 다음 4개의 결과 중 하나가 발생합니다. 
- *성공*: 조치가 정상적으로 호출되었습니다. 
- *애플리케이션 오류*: 조치가 정상적으로 호출되었지만 조치가 의도적으로 오류 값을 리턴했습니다(예: 인수의 전제조건이 충족되지 않음). 
- *조치 개발자 오류*: 조치가 호출되었지만 비정상적으로 완료되었습니다(예: 조치가 예외를 발견하지 못했거나 구문 오류가 존재함). 
- *whisk 내부 오류*: 시스템이 조치를 호출할 수 없습니다.
결과는 다음 절에서 설명하는 대로 활성화 레코드의 `status` 필드에 기록됩니다. 

정상적으로 수신되었으며 사용자가 비용 청구될 수 있는 모든 호출에 대해 활성화 레코드가 있습니다. 

결과가 *조치 개발자 오류*인 경우에는 조치가 부분적으로 실행될 수 있으며 외견상의 부작용이 생성될 수 있습니다. 사용자는 이러한 부작용이 발생했는지 여부를 확인하고, 필요한 경우 재시도 로직을 실행해야 합니다. 특정 *whisk 내부 오류*는 조치가 실행을 시작하지만 조치가 완료를 알리기 전에 실패함을 표시합니다. 

## 활성화 레코드
{: #openwhisk_ref_activation}

각각의 조치 호출과 트리거 실행의 결과로 활성화 레코드가 생성됩니다. 
...
- *logs*: 해당 활성화 중에 조치에 의해 생성된 로그의 문자열 배열입니다. 각각의 배열 요소는 조치에 의한 `stdout` 또는 `stderr`로의 행 출력에 대응되며, 이에는 로그 출력의 시간과 스트림이 포함됩니다. 구조는 다음과 같습니다. `TIMESTAMP STREAM: LOG_OUTPUT`.
- *response*: `success`, `status` 및 `result` 키를 정의하는 사전입니다. 
  - *status*: "성공", "애플리케이션 오류", "조치 개발자 오류" 및 "whisk 내부 오류" 값 중 하나일 수 있는 활성화 결과입니다. 
  - *success*: 상태가 `"success"`인 경우에만 `true`입니다. 
- *result*: 활성화 결과가 포함된 사전입니다. 활성화에 성공한 경우, 결과에는 조치에 의해 리턴된 값이 포함됩니다. 활성화에 실패한 경우에 `result`에는 일반적으로 실패에 대한 설명과 함께 `error` 키가 포함됩니다. 
...
Javascript로 작성된 조치는 단일 파일로 제한되어야 합니다. 파일에는 다수의 함수가 포함될 수 있지만, 조치가 호출될 때 호출되는 함수인 `main`이라고 하는 함수가 관례상 존재해야 합니다. 예를 들어, 다음 예제는 다수의 함수가 있는 조치를 보여줍니다. 
```javascript
function main() {
...
조치 입력 매개변수는 `main` 함수에 대한 매개변수로서 JSON 오브젝트로 전달됩니다. 성공적인 활성화의 결과 역시 JSON 오브젝트이지만, 다음 절에서 설명하는 대로 조치가 동기인지 또는 비동기인지 여부에 따라 다르게 리턴됩니다. 


### 동기 및 비동기 작동
{: #openwhisk_ref_javascript_synchasynch}

일반적으로 JavaScript 함수는 리턴 후에도 콜백 함수에서 계속 실행됩니다. 이러한 작동을 허용하기 위해 Javascript 조치의 활성화는 *동기* 또는 *비동기*일 수 있습니다. 

다음 상태 중 하나에서 main 함수가 존재하는 경우, Javascript 조치의 활성화는 **동기**입니다. 

- main 함수가 `return` 명령문을 실행하지 않고 종료합니다. 
- main 함수가 Promise를 *제외한* 값을 리턴하는 `return` 명령문을 실행하여 종료합니다. 

동기 조치의 다음 예제를 참조하십시오. 
...
main 함수가 Promise를 리턴하여 종료하는 경우, Javascript 조치의 활성화는 **비동기**입니다. 이 경우에 시스템은 Promise가 이행되거나 거부될 때까지 조치가 계속 실행 중이라고 가정합니다.
...
다음 예제에 표시된 대로 일부 입력에서는 조치가 동기이고 기타에서는 비동기일 수 있습니다.  
```javascript
  function main(params) {
      if (params.payload) {
...
활성화가 동기인지 또는 비동기인지 여부와 무관하게, 조치의 호출은 블로킹 또는 넌블로킹일 수 있습니다. 
...
JavaScript 조치는 Node.js 버전 6 또는 Node.js 버전 8에서 실행될 수 있습니다.
현재 조치는 기본적으로 Node.js 버전 6.11.4 환경에서 실행됩니다.   

### Node.js 버전 6 환경
{: #openwhisk_ref_javascript_environments_6}
Node.js 6.12.2 환경은 조치를 작성하거나 업데이트할 때 `--kind` 플래그가 `nodejs:6`의 값으로 명시적으로 지정된 경우에 사용됩니다. 

다음 패키지를 Node.js 6.12.2 환경에서 사용할 수 있습니다. 
...
- [mustache v2.3.0](https://www.npmjs.com/package/mustache) - Mustache.js는 Javascript에서 mustache 템플리트 시스템의 구현입니다. 
...
- [openwhisk v3.11.0](https://www.npmjs.com/package/openwhisk) - OpenWhisk 플랫폼에 대한 Javascript 클라이언트 라이브러리입니다. OpenWhisk API 관련 랩퍼를 제공합니다. 
...
Node.js 버전 8.9.3 환경은 조치를 작성하거나 업데이트할 때 `--kind` 플래그가 `nodejs:8`의 값으로 명시적으로 지정된 경우에 사용됩니다. 

다음 패키지가 Node.js 버전 8.9.3 환경에 사전 설치되어 있습니다. 

  - [apn v2.1.5](https://www.npmjs.com/package/apn) - Apple 푸시 알림 서비스와의 인터페이스를 위한 Node.js 모듈입니다. 
  - [async v2.6.0](https://www.npmjs.com/package/async) - 비동기 함수 관련 작업을 위한 함수를 제공합니다. 
  - [bent v1.1.0](https://www.npmjs.com/package/btoa) - 브라우저의 btoa 함수의 포트입니다. 
  - [btoa v1.1.2](https://www.npmjs.com/package/btoa) - 브라우저의 btoa 함수의 포트입니다. 
  - [cloudant v1.10.0](https://www.npmjs.com/package/cloudant) - 이는 Node.js에 대한 공식 Cloudant 라이브러리입니다. 
  - [commander v2.12.2](https://www.npmjs.com/package/commander) - Node.js 명령행 인터페이스에 대한 완전한 솔루션입니다. 
  - [consul v0.30.0](https://www.npmjs.com/package/consul) - 서비스 검색 및 구성을 포함하는 Consul에 대한 클라이언트입니다. 
  - [cookie-parser v1.4.3](https://www.npmjs.com/package/cookie-parser) - 쿠키 헤더를 구문 분석하고 쿠키 이름으로 키 지정된 오브젝트로 req.cookies를 채웁니다. 
  - [cradle v0.7.1](https://www.npmjs.com/package/cradle) - Node.js에 대한 상위 레벨, 캐싱, CouchDB 클라이언트입니다. 
  - [errorhandler v1.5.0](https://www.npmjs.com/package/errorhandler) - 개발 전용 오류 핸들러 미들웨어입니다. 
  - [glob v7.1.2](https://www.npmjs.com/package/glob) - 별표(*) 등과 같이 쉘이 사용하는 패턴을 사용하여 파일을 일치시킵니다. 
  - [gm v1.23.0](https://www.npmjs.com/package/gm) - 노드에 대한 GraphicsMagick 및 ImageMagick입니다. 
  - [ibm-cos-sdk v1.1.1](https://www.npmjs.com/package/ibm-cos-sdk) - Node.js용 IBM Cloud Object Storage SDK입니다. 
  - [ibm_db v2.2.1](https://www.npmjs.com/package/ibm_db) - IBM DB2 및 IBM Informix에 대한 node.js의 비동기/동기 인터페이스입니다.  
  - [lodash v4.17.4](https://www.npmjs.com/package/lodash) - Node.js 모듈로서 노출된 Lodash 라이브러리입니다. 
  - [log4js v2.3.12](https://www.npmjs.com/package/log4js) - 노드 관련 작업을 위해 디자인된 log4js 프레임워크의 변환입니다. 
  - [iconv-lite v0.4.19](https://www.npmjs.com/package/iconv-lite) - 순수 JS 문자 인코딩 변환입니다. 
  - [marked v0.3.7](https://www.npmjs.com/package/marked) - Javascript로 작성된 전체 기능의 마크다운 구문 분석기 및 컴파일러입니다. 속도를 위해 빌드되었습니다. 
  - [merge v1.2.0](https://www.npmjs.com/package/merge) - 다중 오브젝트를 하나로 병합하며, 선택사항으로 복제된 새 오브젝트를 작성합니다. 
  - [moment v2.19.3](https://www.npmjs.com/package/moment) - 날짜를 구문 분석, 유효성 검증, 조작 및 형식화하기 위한 경량 Javascript 날짜 라이브러리입니다. 
  - [mongodb v2.2.33](https://www.npmjs.com/package/mongodb) - Node.js에 대한 공식 MongoDB 드라이버입니다. 
  - [mustache v2.3.0](https://www.npmjs.com/package/mustache) - Mustache.js는 Javascript에서 mustache 템플리트 시스템의 구현입니다. 
  - [nano v6.4.2](https://www.npmjs.com/package/nano) - Node.js에 대한 최소한의 couchdb 드라이버입니다. 
  - [nodemailer v4.4.1](https://www.npmjs.com/package/nodemailer) - Node.js에서 이메일을 발송합니다. 아주 쉽습니다! 
  - [oauth2-server v3.0.0](https://www.npmjs.com/package/oauth2-server) - Node.js의 표현으로 OAuth2 서버/제공자를 구현하기 위한 완벽하고 호환 가능하며 성공적으로 테스트된 모듈입니다. 
  - [openwhisk v3.11.0](https://www.npmjs.com/package/openwhisk) - OpenWhisk 플랫폼에 대한 Javascript 클라이언트 라이브러리입니다. OpenWhisk API 관련 랩퍼를 제공합니다. 
  - [process v0.11.10](https://www.npmjs.com/package/process) - require('process'). 기타 모듈과 같습니다. 
  - [pug v2.0.0-rc.4](https://www.npmjs.com/package/pug) - Pug 템플레이팅 언어를 구현합니다. 
  - [redis v2.8.0](https://www.npmjs.com/package/redis) - Node.js에 대한 완전하고 기능이 풍부한 Redis 클라이언트입니다. 
  - [request v2.83.0](https://www.npmjs.com/package/request) - 요청은 HTTP 호출 작성을 위한 가능한 가장 단순한 방법입니다. 
  ...
  - [semver v5.4.1](https://www.npmjs.com/package/semver) - 시맨틱 버전화를 지원합니다. 
  - [@sendgrid/mail@6.1.4](https://www.npmjs.com/package/@sendgrid/mail) - SendGrid API를 통한 이메일 지원을 제공합니다. 
  - [serve-favicon v2.4.5](https://www.npmjs.com/package/serve-favicon) - 파비콘을 서비스하기 위한 Node.js 미들웨어입니다. 
  - [socket.io v2.0.4](https://www.npmjs.com/package/socket.io) - Socket.IO는 실시간 양방향 이벤트 기반 통신을 가능하게 합니다. 
  - [socket.io-client v2.0.4](https://www.npmjs.com/package/socket.io-client) - Socket.IO에 대한 클라이언트 측 지원입니다. 
  - [superagent v3.8.2](https://www.npmjs.com/package/superagent) - SuperAgent는 소형의 혁신적 클라이언트 측 HTTP 요청 라이브러리 및 동일한 API의 Node.js 모듈이며, 다수의 상위 레벨 HTTP 클라이언트 기능을 과시합니다. 
  - [swagger-tools v0.10.3](https://www.npmjs.com/package/swagger-tools) - API를 문서화하는 방식인 Swagger 관련 작업과 연관된 도구입니다. 
  - [tmp v0.0.33](https://www.npmjs.com/package/tmp) - node.js에 대한 단순 임시 파일 및 디렉토리 작성자입니다. 
  - [twilio v3.10.1](https://www.npmjs.com/package/twilio) - 음성, 비디오 및 메시징과 관련된 Twilio API에 대한 랩퍼입니다. 
  - [underscore v1.8.3](https://www.npmjs.com/package/underscore) - Underscore.js는 유력한 기능상 용의자(각각, 맵, 축소, 필터...)를 지원하는 Javascript용 유틸리티 벨트 라이브러리입니다. 코어 Javascript 오브젝트의 확장은 없습니다. 
  - [uuid v3.1.0](https://www.npmjs.com/package/uuid) - RFC4122 UUIDS의 단순하고 신속한 생성입니다. 
  - [validator v9.2.0](https://www.npmjs.com/package/validator) - 문자열 유효성 검증기 및 무결 처리기의 라이브러리입니다. 
  - [watson-developer-cloud v3.0.2](https://www.npmjs.com/package/watson-developer-cloud) - 코그너티브 컴퓨팅을 사용하여 복잡한 문제점을 해결하는 API의 콜렉션인 Watson Developer Cloud 서비스를 사용하기 위한 Node.js 클라이언트 라이브러리입니다. 
  ...
  - [winston v2.4.0](https://www.npmjs.com/package/winston) - node.js에 대한 다중 전송 비동기 로깅 라이브러리입니다. "CHILL WINSTON! ... I put it in the logs."
  - [ws v3.3.1](https://www.npmjs.com/package/ws) - ws는 사용이 단순하고 매우 빠르며 철저한 테스트를 거친 WebSocket 클라이언트 및 서버 구현입니다. 
...
### 사용자 조치로 npm 패키지를 패키징
Node.js 환경에 사전 설치되지 않은 `npm` 패키지의 경우에는 조치를 작성하거나 업데이트할 때 이를 종속 항목으로 번들링할 수 있습니다. 

자세한 정보는 [Node.js 모듈로서 조치 패키징](./openwhisk_actions.html#openwhisk_js_packaged_action) 또는 [단일 번들로서 조치 패키징](./openwhisk_actions.html#openwhisk_js_webpack_action)을 참조하십시오. 


## Python 런타임 환경
{: #openwhisk_ref_python_environments}

OpenWhisk는 두 개의 서로 다른 런타임 버전을 사용하여 Python 조치의 실행을 지원합니다. 


### Python 3 조치(Jessie 기반)
{: #openwhisk_ref_python_environments_jessie}

Python 3 조치는 Python 3.6.4에서 실행됩니다. 이 런타임을 사용하려면 조치를 작성하거나 업데이트할 때 `wsk` CLI 매개변수 `--kind python-jessie:3`을 지정하십시오.
virtualenv를 사용하여 python 조치를 작성할 때는 Docker 이미지 `ibmfunctions/action-python-v3`을 사용하십시오.
런타임에는 Python 3.6 표준 라이브러리에 추가하여 Python 조치에서 사용할 수 있는 IBM Cloud 서비스에 대한 SDK 패키지가 포함되어 있습니다. 
...
### Python 3 조치(Alpine 기반)
{: #openwhisk_ref_python_environments_alpine}

Python 3 조치는 Python 3.6.1에서 실행됩니다. 이 런타임을 사용하려면 조치를 작성하거나 업데이트할 때 `wsk` CLI 매개변수 `--kind python:3`을 지정하십시오.
virtualenv를 사용하여 python 조치를 작성할 때는 Docker 이미지 `openwhisk/python3action`을 사용하십시오.
Python 3.6 표준 라이브러리에 추가하여 다음 패키지를 Python 조치에서 사용할 수 있습니다. 
...
Python 2 조치는 Python 조치에 대한 기본 런타임인 Python 2.7.12에서 실행됩니다. 조치를 작성하거나 업데이트할 때 `--kind` 플래그를 지정하지 않는 한, 이 런타임을 명시적으로 선택하려면 `--kind python:2`를 사용하십시오.
virtualenv를 사용하여 python 조치를 작성할 때는 Docker 이미지 `openwhisk/python2action`을 사용하십시오.
Python 2.7 표준 라이브러리에 추가하여 다음 패키지를 Python 2 조치에서 사용할 수 있습니다. 
...
### Swift 3
Swift 3 조치는 Swift 3.1.1 `--kind swift:3.1.1`에서 실행됩니다. 이전 Swift 버전이 지원되지 않으므로 항상 `swift:3.1.1` 유형을 지정하십시오. 

`swift:3.1.1` 유형을 사용하려면 모든 Swift 조치를 마이그레이션해야 합니다. 조치를 작성하거나 업데이트할 때 항상 특정 유형을 제공하는 것이 가장 좋습니다.
{: tip}

Swift 3.1.1 조치는 다음 패키지를 사용할 수 있습니다. 
...
PHP 조치는 PHP 7.1에서 실행됩니다. 이 런타임을 사용하려면 조치를 작성하거나 업데이트할 때 `wsk` CLI 매개변수 `--kind php:7.1`을 지정하십시오. `.php` 확장자가 있는 파일로 조치를 작성할 때는 이 작동이 기본 작동입니다. 
...
Docker 조치는 Docker 컨테이너에서 사용자 제공 2진을 실행합니다. 2진이 [python:2.7.12-alpine](https://hub.docker.com/r/library/python) 기반의 Docker 이미지에서 실행되므로, 2진은 이 배포와 호환 가능해야 합니다. 

Docker 스켈레톤은 OpenWhisk 호환 가능 Docker 이미지를 빌드하는 편리한 방법입니다. `wsk sdk install docker` CLI 명령을 사용하여 스켈레톤을 설치할 수 있습니다.

기본 2진 프로그램은 컨테이너 내부의 `/action/exec`에 있어야 합니다. 실행 파일은 `JSON` 오브젝트로서 역직렬화될 수 있는 단일 명령행 인수 문자열을 통해 입력 인수를 수신합니다. 이는 직렬화된 `JSON`의 단일행 문자열로서 `stdout`을 통해 결과를 리턴합니다. 
...
### 조치
{{site.data.keyword.openwhisk_short}}에는 일부 시스템 한계가 있으며, 이에는 조치에서 사용할 수 있는 메모리 양과 분당 허용된 조치 호출 수가 포함됩니다. 

다음 표에는 조치에 대한 기본 한계가 나열되어 있습니다. 

| 한계 | 설명 | 기본값 | 최소값 | 최대값 | 
| ----- | ----------- | :-------: | :---: | :---: |
| [codeSize](openwhisk_reference.html#openwhisk_syslimits_codesize) | 조치 코드의 최대 크기(MB)입니다. | 48 | 1 | 48 |
...
| [openulimit](openwhisk_reference.html#openwhisk_syslimits_openulimit) | Docker 조치에 대한 열린 파일의 최대 수입니다. | 64 | 0 | 64 |
| [parameters](openwhisk_reference.html#openwhisk_syslimits_parameters) | 첨부될 수 있는 매개변수의 최대 크기(MB)입니다. | 1 | 0 | 1 |
| [proculimit](openwhisk_reference.html#openwhisk_syslimits_proculimit) | Docker 조치가 사용할 수 있는 최대 프로세스 수입니다. | 512 | 0 | 512 |
| [result](openwhisk_reference.html#openwhisk_syslimits_result) | 조치 호출 결과의 최대 크기(MB)입니다. | 1 | 0 | 1 |
| [timeout](openwhisk_reference.html#openwhisk_syslimits_timeout) | 컨테이너가 N밀리초 넘게 실행될 수 없습니다. | 60000 | 100 | 300000 |
...
* 조치에 대한 최대 코드 크기는 48MB입니다. 
* Javascript 조치에서 종속 항목이 포함된 모든 소스 코드를 단일 번들링된 파일에 연결하는 도구를 사용하도록 권장합니다. 
...
* 조치가 작성되거나 업데이트될 때 사용자는 조치 로그 한계를 변경할 수 있습니다. 
* 설정 한계를 초과하는 로그는 잘립니다. 따라서 새 로그 항목은 무시되며 경고는 활성화가 설정된 로그 한계를 초과했음을 표시하기 위해 활성화의 마지막 출력으로서 추가됩니다. 

#### memory(MB)(기본값: 256MB)
{: #openwhisk_syslimits_memory}
* 메모리 한계 M은 [128MB..512MB] 범위이며 MB 단위로 조치당 설정됩니다. 
* 사용자는 조치가 작성될 때 메모리 한계를 변경할 수 있습니다. 
* 컨테이너는 한계에 의해 할당된 메모리를 초과하여 사용할 수 없습니다. 

#### minuteRate(고정: 5000*)
{: #openwhisk_syslimits_minuterate}
* 속도 제한 N은 5000으로 설정되며 1분 창에서 조치 호출의 수를 제한합니다. 
* 이 한계를 초과하는 CLI 또는 API 호출은 HTTP 상태 코드 `429: TOO MANY REQUESTS`에 대응하는 오류 코드를 수신합니다. 
* 이 한계 값은 고정되어 있지만, 경영 사례에서 보다 높은 안전 한계 값을 정당화할 수 있으면 이를 늘릴 수 있습니다. 이 한계를 늘리는 방법에 대한 세부 지시사항은 [고정 한계 늘리기](openwhisk_reference.html#increase_fixed_limit) 절을 참조하십시오. 

#### openulimit(고정: 1024:1024)
{: #openwhisk_syslimits_openulimit}
* Docker 조치에 대한 열린 파일의 최대 수는 (하드 및 소프트 한계 모두에 대해) 1024개입니다. 
* 이 한계는 고정되어 있으며 변경될 수 없습니다. 
* Docker 실행 명령은 `--ulimit nofile=1024:1024` 인수를 사용합니다. 
* 자세한 정보는 [docker run](https://docs.docker.com/engine/reference/commandline/run) 명령행 참조 문서를 참조하십시오. 

#### parameters(고정: 1MB)
{: #openwhisk_syslimits_parameters}
* 조치/패키지/트리거의 작성 및 업데이트 시에 총 매개변수의 크기 한계는 1MB입니다. 
* 너무 큰 매개변수의 엔티티는 이의 작성 또는 업데이트 시에 거부됩니다. 
* 이 한계는 고정되어 있으며 변경될 수 없습니다. 

#### proculimit(고정: 1024:1024)
{: #openwhisk_syslimits_proculimit}
* Docker 조치 컨테이너가 사용할 수 있는 프로세스의 최대 수는 1024입니다. 
* 이 한계는 고정되어 있으며 변경될 수 없습니다. 
* Docker 실행 명령은 `--pids-limit 1024` 인수를 사용합니다. 
* 자세한 정보는 [docker run](https://docs.docker.com/engine/reference/commandline/run) 명령행 참조 문서를 참조하십시오. 

#### result(고정: 1MB)
{: #openwhisk_syslimits_result}
* 조치 호출 결과의 최대 출력 크기(MB)입니다. 
* 이 한계는 고정되어 있으며 변경될 수 없습니다. 

#### timeout(ms)(기본값: 60s)
{: #openwhisk_syslimits_timeout}
* 제한시간 한계 N은 [100ms..300000ms] 범위이며 밀리초로 조치당 설정됩니다. 
* 사용자는 조치가 작성될 때 제한시간 한계를 변경할 수 있습니다. 
...
| 한계 | 설명 | 기본값 | 최소값 | 최대값 | 
| ----- | ----------- | :-------: | :---: | :---: |
| [minuteRate](openwhisk_reference.html#openwhisk_syslimits_tminuterate) | N개가 넘는 트리거가 분당 네임스페이스마다 실행될 수 없습니다. | 5000* | 5000* | 5000* |
...
* 속도 제한 N은 5000으로 설정되며 1분 창에서 사용자가 실행할 수 있는 트리거의 수를 제한합니다. 
