...
서버리스 아키텍처가 모든 컴퓨팅 문제점의 만병통치약은 아니지만, 그래도 이는 일부 문제점을 해결합니다. 서버리스 디자인이 좋은 선택사항이 될 수 있는 [많은 유스 케이스](./openwhisk_use_cases.html)가 있습니다. 다음 아키텍처가 비교 대상입니다.
{: shortdesc}

1. **FaaS(Function as a Service)** - 관리 대상 OpenWhisk. IBM은 관리 대상인 [{{site.data.keyword.Bluemix_notm}}의 OpenWhisk](https://console.ng.bluemix.net/openwhisk)를 제공하는 유일한 공급업체입니다. 

2. OpenWhisk RYO(Roll Your Own)가 포함된 **IaaS(Infrastructure as a Service)**. 일반 사용자는 Apache Incubation Project에서 OpenWhisk를 다운로드할 수 있으며 [{{site.data.keyword.Bluemix_notm}} IaaS](https://console.ng.bluemix.net/catalog/?category=devices) 및 기타 [IaaS 클라우드](https://en.wikipedia.org/wiki/Cloud_computing#Infrastructure_as_a_service_.28IaaS.29)에 이를 설치하고 실행할 수 있습니다. 

3. **PaaS(Platform as a Service)** - 관리 대상 애플리케이션 런타임. {{site.data.keyword.Bluemix_notm}} CloudFoundry 구현에 의해 관리되는 [Liberty for Java](https://console.ng.bluemix.net/catalog/starters/liberty-for-java) 런타임이 좋은 예입니다. 

4. **CaaS(Container as a Service)** - 관리 대상 컨테이너 환경. IBM의 [{{site.data.keyword.Bluemix_notm}}의 컨테이너](https://console.ng.bluemix.net/catalog/?category=containerImages)가 좋은 예입니다. 

5. Java EE 런타임이 포함된 **IaaS(Infrastructure as a Service)**. IBM의 [{{site.data.keyword.Bluemix_notm}}의 WebSphere Application Server VM](https://console.ng.bluemix.net/catalog/services/websphere-application-server)이 좋은 예입니다. 

각 아키텍처의 장단점에 대한 요약이 이러한 서로 다른 런타임에서 애플리케이션을 개발하고 운용 중인 **일반 사용자의 관점**에서 제공됩니다. 
...
|	탄력적 스케일링 |	각각의 조치는 항상 로드에 따라 즉시 기본적으로 스케일링됩니다. 사전에 VM 또는 기타 리소스를 프로비저닝할 필요가 없습니다. |	제공되지 않음 - 일반 사용자가 IaaS의 컴퓨팅 용량을 제공하고 VM의 스케일링을 관리해야 합니다. 일단 VM이 스케일링되면 OpenWhisk가 조치를 자동으로 스케일링하지만, 리소스는 사전에 이미 프로비저닝되어 있어야 합니다. |	자동적이지만 느린 스케일링입니다. 로드가 증가하는 동안 사용자는 수 분 정도 스케일링 조치가 완료될 때까지 대기해야 할 수 있습니다. 자동 스케일링에서는 신중한 튜닝이 필요합니다. |	자동적이지만 느린 스케일링입니다. 로드가 증가하는 동안 사용자는 수 분 정도 스케일링 조치가 완료될 때까지 대기해야 할 수 있습니다. 자동 스케일링에서는 신중한 튜닝이 필요합니다. |	제공되지 않음 |
...
|	유지보수 |	없음 - 전체 스택을 IBM이 관리합니다. |	과중함 - 대상 환경에 따라 사용자는 하드웨어, 네트워킹, OS, 스토리지, DB, OpenWhisk 설치 및 유지보수 등을 프로비저닝해야 합니다. |	없음 - 전체 스택을 공급업체가 관리합니다. |	과중함 - 사용자가 사용자 정의 이미지를 작성하고 유지보수해야 하며 컨테이너 배치 및 관리, 컨테이너 간의 연결 등을 수행해야 합니다. |	상당함 - 사용자가 VM을 할당하고 Java EE 서버를 개별적으로 관리 및 스케일링해야 합니다. |
