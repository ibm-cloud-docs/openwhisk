...
이 서비스는 다양한 언어와 오디오 형식으로부터 음성을 필사할 수 있습니다. 기본 필사 외에도 이 서비스는 오디오의 다양한 측면에 대한 상세 정보를 생성할 수 있습니다. 대부분의 언어에서, 이 서비스는 2개의 샘플링 속도인 광대역 및 협대역을 지원합니다. 이는 UTF-8 문자 세트의 모든 JSON 응답 컨텐츠를 리턴한다. 이 서비스에 대한 자세한 정보는 [IBM&reg; Cloud 문서](https://cloud.ibm.com/docs/services/speech-to-text/index.html)를 참조하십시오.
