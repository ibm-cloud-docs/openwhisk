...
L'SDK mobile non è supportato per gli spazi dei nomi basati su IAM. Utilizza invece uno spazio dei nomi basato su Cloud Foundry.
{: important}
...
Il codice sorgente è disponibile all'indirizzo https://github.com/apache/incubator-openwhisk-client-swift.
...
Questo comando scarica un file compresso che contiene l'applicazione starter. La directory del progetto contiene un Podfile.
...
Per iniziare subito a lavorare, crea un oggetto `WhiskCredentials` con le tue credenziali dell'API OpenWhisk e crea un'istanza OpenWhisk dall'oggetto.
...
La stringa prima dei due punti è la tua chiave e la stringa dopo i due punti è il tuo token.
...
In questo esempio, utilizzi un'installazione in esecuzione su `http://localhost:8080`. Se non specifichi il parametro `baseURL`, l'SDK mobile utilizza l'istanza in esecuzione su https://us-south.functions.cloud.ibm.com.
...
- `qName = "foo"` dà come risultato `namespace = default`, `package = default`, `action/trrigger = "foo"`
- `qName = "mypackage/foo"` dà come risultato `namespace = default`, `package = mypackage`, `action/trigger = "foo"`
- `qName = "/mynamespace/foo"` dà come risultato `namespace = mynamespace`, `package = default`, `action/trigger = "foo"`
- `qName = "/mynamespace/mypackage/foo"` dà come risultato `namespace = mynamespace`, `package = mypackage`, `action/trigger = "foo"`
