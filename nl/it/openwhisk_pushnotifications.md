...
# Utilizzo del pacchetto Push Notification
{: #openwhisk_catalog_pushnotifications}

Il pacchetto `/whisk.system/pushnotifications` ti offre la possibilità di lavorare con un servizio push.
{: shortdesc}
...
-  `appId`: il GUID dell'applicazione {{site.data.keyword.Bluemix}}.
-  `appSecret`: l'appSecret del servizio di notifica di push {{site.data.keyword.Bluemix_notm}}.

Per creare un bind di pacchetto, vedi la seguente procedura di esempio:
...
## Attivazione di un evento trigger sull'attività del servizio Push Notification
{: #openwhisk_catalog_pushnotifications_fire}

`/whisk.system/pushnotifications/webhook` configura il servizio Push per attivare un trigger quando, in un'applicazione specificata, si verifica un'attività del dispositivo, come ad esempio la registrazione/l'annullamento della registrazione o la sottoscrizione/l'annullamento della sottoscrizione del dispositivo

I parametri sono i seguenti:

- `appId:` il GUID dell'applicazione {{site.data.keyword.Bluemix_notm}}.
- `appSecret:` l'appSecret del servizio di notifica di push {{site.data.keyword.Bluemix_notm}}.
- `events:` gli eventi supportati sono `onDeviceRegister`, `onDeviceUnregister`, `onDeviceUpdate`, `onSubscribe` e `onUnsubscribe`. Puoi utilizzare il carattere jolly `*` per ricevere una notifica per tutti gli eventi.
...
1. Crea un bind di pacchetto configurato per il tuo servizio Push Notification con i tuoi appId e appSecret.
  ```
...
3. Potresti creare una regola che invii un messaggio ogni volta che si registra un nuovo dispositivo. Crea una regola utilizzando l'azione e il trigger precedenti.
  ```
  ...
  Registra un dispositivo nella tua applicazione {{site.data.keyword.Bluemix_notm}}. Puoi vedere che `Rule`, `Trigger` e `Action` vengono eseguiti nel [dashboard](https://console.{Domain}/openwhisk/dashboard) OpenWhisk.
