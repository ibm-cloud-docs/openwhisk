...
通过 `/whisk.system/pushnotifications` 包，您可以使用 Push 服务。
{: shortdesc}
...
2. 初始化推送通知服务，并将该服务绑定到 {{site.data.keyword.Bluemix_notm}} 应用程序。
...
`/whisk.system/pushnotifications/webhook` 将 Push 服务配置为在指定应用程序中存在设备活动（如设备注册/取消注册或预订/取消预订）时触发触发器。

参数如下所示：

- `appId`：{{site.data.keyword.Bluemix_notm}} 应用程序 GUID。
- `appSecret`：{{site.data.keyword.Bluemix_notm}} 推送通知服务 appSecret。
- `events`：受支持的事件为 `onDeviceRegister`、`onDeviceUnregister`、`onDeviceUpdate`、`onSubscribe`、`onUnsubscribe`。可以使用通配符 `*` 来接收所有事件的通知。
...
3. 可以创建规则，用于在每次新设备注册时发送消息。使用先前的操作和触发器创建规则。
  ...
  在 {{site.data.keyword.Bluemix_notm}} 应用程序中注册设备。您可以看到`规则`、`触发器`和`操作`在 OpenWhisk [仪表板](https://console.{Domain}/openwhisk/dashboard)中执行。
