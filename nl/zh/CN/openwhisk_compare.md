...
1. [{{site.data.keyword.openwhisk_short}}](https://cloud.ibm.com/openwhisk) 上的**功能即服务 (FaaS)**。IBM 是唯一一家提供受管 OpenWhisk 的供应商。[Martin Fowler 的博客](https://martinfowler.com/articles/serverless.html)上详细介绍了使用 FaaS 平台的无服务器编程模型，您可以查看运行采用无服务器设计的 OpenWhisk 的[用例](/docs/openwhisk?topic=cloud-functions-openwhisk_common_use_cases)。
...
4. **容器即服务 (CaaS)** 作为受管容器环境。[{{site.data.keyword.containerlong_notm}}](/docs/containers?topic=containers-container_index#container_index) 就是一个很好的例子。
