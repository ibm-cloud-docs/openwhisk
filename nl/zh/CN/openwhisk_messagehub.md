...
此包支持与 [Message Hub](https://developer.ibm.com/messaging/message-hub) 实例进行通信，以通过本机高性能 Kafka API 来发布和使用消息。
{: shortdesc}
...
|名称|类型|描述|
|---|---|---|
|kafka_brokers_sasl|字符串的 JSON 数组|此参数是一组 `<host>:<port>` 字符串，包含 Message Hub 实例中的代理程序|
...
2. 验证要侦听的主题在 Message Hub 中是否可用，或者创建新主题，例如 `mytopic`。
...
## 在 {{site.data.keyword.Bluemix_notm}} 外部设置 Message Hub 包

如果要在 {{site.data.keyword.Bluemix_notm}} 外部设置 Message Hub，那么必须为 Message Hub 服务手动创建包绑定。您需要 Message Hub 服务凭证和连接信息。
...
该触发器的有效内容将包含 `messages` 字段，此字段是自上次触发触发器以来已发布的消息数组。数组中的每个消息对象都包含以下字段：
- topic
- partition
- offset
- key
...
前三个参数可使用 `wsk package refresh` 自动绑定，请参阅以下使用所有必需参数调用操作的示例：
