...
显示的信息提供了 HTTP 请求的属性，并使用 User-Agent 头 `OpenWhisk-CLI/1.0 (<CLI-Build-version>)` 和基本授权头 `Basic XXXYYYY` 对 URL `https://openwhisk.ng.bluemix.net/api/v1/namespaces` 执行 HTTP 方法 `GET`。
