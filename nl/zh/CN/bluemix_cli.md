...
有关 plugin 命令的更多信息，请使用 `ibmcloud plugin --help` 或查看以下文档：
...
`-a` 标志指定要使用的 {{site.data.keyword.Bluemix_notm}} API 端点。如果已指定 API 端点，那么 `-a` 选项不是必需的。可以使用 `ibmcloud api` 命令来显式设置 {{site.data.keyword.Bluemix_notm}} API 端点。要显示当前的 API 端点设置，请使用 `ibmcloud target` 命令。
...
有关 login 命令的更多信息，请使用 `ibmcloud login --help` 或查看以下文档：
...
要获取 {{site.data.keyword.openwhisk_short}} 插件的命令列表，请运行不带任何自变量的 `ibmcloud wsk`。
...
可以使用 `ibmcloud iam space-create` 在您的组织下创建更多空间，例如“staging”和“production”。请参阅以下示例：
```
ibmcloud iam space-create "staging"
ibmcloud iam space-create "production"
```

有关 `ibmcloud iam` 命令的更多信息，请参阅以下文档：https://console.bluemix.net/docs/cli/reference/bluemix_cli/bx_cli.html#bluemix_iam_space_create
...
使用 `ibmcloud target` 命令可更改区域。例如，如果要切换到英国区域和空间 `staging`，请运行以下命令：
```
...
有关 `target` 命令的更多信息，请使用 `ibmcloud target --help` 或查看以下文档：
...
OpenWhisk CLI 需要配置认证 API 密钥和 API 主机。使用 {{site.data.keyword.openwhisk_short}} CLI 插件时，您无需显式配置 API 密钥和 API 主机。您需要使用 `ibmcloud login` 登录，然后使用 `ibmcloud target` 命令将您的区域和名称空间设定为目标。在此之后，所有后续命令都以 `ibmcloud wsk` 开头。以这种方式进行认证还可避免访问 {{site.data.keyword.Bluemix_notm}} Web 页面来获取用于配置 CLI 环境的特定信息。
...
使用 {{site.data.keyword.openwhisk}} CLI 插件后，不再需要运行 `wsk bluemix login`。请改为使用 `ibmcloud login` 命令登录到 {{site.data.keyword.Bluemix_notm}}，随后 {{site.data.keyword.openwhisk}} 插件会自动利用您当前的登录和目标信息。现在，您可以使用 `ibmcloud wsk api` 命令来管理 API。

### 迁移部署脚本
{: #migrating_deploy_scripts}

如果您具有将 OpenWhisk CLI 与 `wsk` 二进制文件一起使用的脚本，那么所有命令的工作方式与使用 `ibmcloud wsk` 命令相同。您可以修改脚本以使用 {{site.data.keyword.Bluemix_notm}} CLI 插件，或者创建别名或包装程序，使当前执行的 `wsk` 转换为 `ibmcloud wsk`。{{site.data.keyword.Bluemix_notm}} CLI 中的 `ibmcloud login` 和 `ibmcloud target` 命令以无人照管方式工作。在无人照管方式下，可以先配置环境，然后再执行 `ibmcloud wsk` 命令来部署和管理 {{site.data.keyword.openwhisk_short}} 实体。
...
* 现在，`ibmcloud wsk api` 接受路径参数，例如 `/api/{id}`。有关信息，请参阅 [API 网关](./openwhisk_apigateway.html)
* 恢复了代理支持。
* 除去了 `swift:3`。
