...
# {{site.data.keyword.discoveryshort}} 套件

{{site.data.keyword.discoveryfull}} 服務是認知搜尋及內容分析引擎，您可以將其新增至應用程式，以識別型樣、趨勢及可採取動作的洞察，而產生更好的決策。將結構化及非結構化資料與預先強化的內容安全地統一，並使用簡化的查詢語言來省掉手動過濾結果這一程序。
{: shortdesc}

{{site.data.keyword.discoveryshort}} 套件包含下列實體。您可以按一下實體名稱，在 {{site.data.keyword.discoveryshort}} API 參照中找到其他詳細資料。
