...
如需外掛程式指令的相關資訊，請使用 `bx plugin --help` 或查看下列文件：
...
`-a` 旗標指定要使用的 {{site.data.keyword.Bluemix_notm}} API 端點。如果指定 API 端點，則不需要 `-a` 選項。您可以使用 `bx api` 指令來明確設定 {{site.data.keyword.Bluemix_notm}} API 端點。若要顯示現行 API 端點設定，請使用 `bx target` 指令。
...
如需登入指令的相關資訊，請使用 `bx login --help` 或檢閱下列文件：
...
若要取得 {{site.data.keyword.openwhisk_short}} 外掛程式的指令清單，請執行 `bx wsk`（不含引數）。
...
您可以使用 `bx iam space-create`，在組織下建立更多空間，例如，"staging" 和 "production"。請參閱下列範例：
```
bx iam space-create "staging"
bx iam space-create "production"
```

如需 `bx iam` 指令的相關資訊，請參閱下列文件： https://console.bluemix.net/docs/cli/reference/bluemix_cli/bx_cli.html#bluemix_iam_space_create
...
如果您已登入，則可以在 {{site.data.keyword.Bluemix_notm}} CLI 中執行 `bx target` 指令，以切換地區、組織及空間。
...
使用 `bx target` 指令可變更地區。例如，若您想要切換至英國地區以及空間 `staging`：
```
...
如需 `target` 指令的相關資訊，請使用 `bx target --help` 或檢閱下列文件：
...
除了不再需要的 `wsk bluemix login` 指令之外，所有 `wsk` 指令都使用 `bx wsk` 指令，以相同的方式運作。所有指令選項和引數都相同。

### API 鑑別及主機
{: #api_authentication}

OpenWhisk CLI 要求您配置鑑別 API 金鑰及 API 主機。
使用 {{site.data.keyword.openwhisk_short}} CLI 外掛程式，您不需要明確配置 API 金鑰及 API 主機。您需要使用 `bx login` 來登入，並使用 `bx target` 指令，將您的地區和名稱空間設為目標。之後，所有後續指令的開頭都是 `bx wsk`。以此方式鑑別也可避免存取 {{site.data.keyword.Bluemix_notm}} 網頁，以免取得配置 CLI 環境的特定資訊。
...
使用 {{site.data.keyword.openwhisk}} CLI 外掛程式，不再需要執行 `wsk bluemix login`。而是改用 `bx login` 指令來登入 {{site.data.keyword.Bluemix_notm}}，{{site.data.keyword.openwhisk}} 外掛程式會自動利用現行登入和目標資訊。現在，您可以使用 `bx wsk api` 指令來管理您的 API。

### 移轉部署 Script
{: #migrating_deploy_scripts}

如果您的 Script 使用具有 `wsk` 二進位的 OpenWhisk CLI，則所有指令都使用 `bx wsk` 指令，以相同的方式來運作。您可以修改 Script 來使用 {{site.data.keyword.Bluemix_notm}} CLI 外掛程式，或建立別名或封套，使現行的 `wsk` 執行轉換為 `bx wsk`。{{site.data.keyword.Bluemix_notm}} CLI 中的 `bx login` 和 `bx target` 指令以自動模式運作。使用自動模式，您可以在執行 `bx wsk` 指令以進行部署及管理 {{site.data.keyword.openwhisk_short}} 實體之前，先配置環境。
...
1.0.7 (2018-02-02)
* `bx wsk api` 現在接受路徑參數，例如，`/api/{id}`。如需相關資訊，請參閱 [API 閘道](./openwhisk_apigateway.html)
* 還原 Proxy 支援。
* 移除 `swift:3`。

1.0.6 (2018-01-30)
* 針對套件內的動作，在 `bx wsk service bind` 指令中修正錯誤。
