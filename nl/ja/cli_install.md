...
* トリガーおよびルールを使用して、アクションでイベントに応答できるようにする。 『[トリガーとルールの作成](/docs/openwhisk?topic=cloud-functions-triggers)』を参照してください。
* パッケージでアクションをバンドルする方法および外部イベント・ソースを構成する方法を学習する。 『[パッケージの作成と使用](/docs/openwhisk?topic=cloud-functions-pkg_ov)』を参照してください。
* パッケージ・カタログを検討し、[{{site.data.keyword.cloudant}} イベント・ソース](/docs/openwhisk?topic=cloud-functions-pkg_cloudant)などの外部サービスでアプリケーションを強化する。
...
実動前 (ステージング) デプロイメントと実動デプロイメントを処理するための IAM ベースのスペースを作成できます。デプロイメントごとにスペースを作成します。スペースを作成することで、{{site.data.keyword.openwhisk_short}} で 2 つの異なる名前空間が定義されるようにすることができます。 組織の下に「staging」や「production」などのスペースをさらに作成するには、[`ibmcloud iam space-create`](/docs/cli/reference/ibmcloud?topic=cloud-cli-ibmcloud_commands_account#ibmcloud_account_space_create) を実行します。
...
OpenWhisk CLI では、認証 API キーおよび API ホストを構成する必要がありました。 {{site.data.keyword.openwhisk_short}} CLI プラグインでは、API キーおよび API ホストを明示的に構成する必要はありません。 代わりに、`ibmcloud login` でログインできます。`ibmcloud fn property set --namespace NAME` を実行して IAM 対応の名前空間をターゲットにするか、`ibmcloud target -o ORG -s SPACE` を実行して Cloud Foundry ベースの名前空間をターゲットにすることができます。ログイン後は、すべてのコマンドを `ibmcloud fn` で開始します。
...
1.0.7 (2018-02-02)
* `ibmcloud wsk api` が更新され、`/api/{id}` などのパス・パラメーターを指定できるようになりました。 詳しくは、『[API ゲートウェイ](/docs/openwhisk?topic=cloud-functions-apigateway)』を参照。
* プロキシー・サポートが復元されました。
* `swift:3` が削除されました。
