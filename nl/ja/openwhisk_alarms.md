...
| `/whisk.system/alarms/once` | フィード | date、trigger_payload | 特定の日付に 1 回、トリガー・イベントを起動します。|
...
`/whisk.system/alarms/interval` フィードは、間隔ベースのスケジュールでトリガー・イベントを起動するようにアラーム・サービスを構成します。パラメーターは以下のとおりです。

- `minutes`: トリガー起動の間隔の長さ (分) を表す整数。

- `trigger_payload`: このパラメーターの値は、トリガーが起動するごとにトリガーの内容になります。

- `startDate`: 最初のトリガーが起動する日付。後続の起動は、`minutes` パラメーターで指定されている間隔の長さに基づいて発生します。   

- `stopDate`: トリガーの実行を停止する日付。この日付に達すると、トリガーは起動しなくなります。
...
`/whisk.system/alarms/once` フィードは、特定の日付にトリガー・イベントを起動するようにアラーム・サービスを構成します。パラメーターは以下のとおりです。

- `date`: トリガーを起動する日付。トリガーは、指定時刻に 1 回だけ起動します。 
...
- `trigger_payload`: このパラメーターの値は、トリガーの起動時におけるトリガーの内容になります。 

以下は、2017 年 12 月 25 日 12:30:00 UTC に 1 回起動するトリガーを作成する例です。
...
`/whisk.system/alarms/alarm` フィードは、指定頻度でトリガー・イベントを起動するようにアラーム・サービスを構成します。パラメーターは以下のとおりです。

- `cron`: 協定世界時 (UTC) でトリガーの起動タイミングを示す、UNIX crontab 構文に基づいたストリング。このストリングは、`X X X X X` のようにスペースで区切られた 5 個のフィールドのシーケンスです。
...
- `startDate`: トリガーの実行を開始する日付。トリガーは、cron パラメーターで指定されたスケジュールに基づいて起動します。  

- `stopDate`: トリガーの実行を停止する日付。トリガーは、この日付に達すると、起動しなくなります。
