...
{{site.data.keyword.openwhisk_short}} はオープンな API をサポートしています。この API では、どのユーザーでも**パッケージ**内の**フィード**としてイベント・プロデューサー・サービスを公開できます。このセクションでは、ユーザー独自のフィードを提供する際に選択できる、アーキテクチャーや実装に関するいくつかのオプションについて説明します。
{: shortdesc}
...
### ポーリング
ポーリング・パターンでは、定期的にエンドポイントをポーリングして新しいデータをフェッチするよう {{site.data.keyword.openwhisk_short}} *アクション* が調整されます。このパターンは、比較的簡単に構築できますが、イベントの頻度はポーリング間隔によって制限されます。
...
フィード・アクションは、フィードを管理するために必要な他のパラメーターも受け入れることができます。例えば、Cloudant の changes フィード・アクションは、*「dbname」*、*「username」* などのパラメーターを受け取ることを予期しています。
...
*changes* という名前のフィード・アクションは、これらのパラメーターを受け取り、Cloudant からのイベントのストリームをセットアップするために必要なアクションを実行すると予期されます。フィード・アクションは適切な構成を使用することによって発生し、それがトリガー *T* に送信されます。    

Cloudant *changes* フィードの場合は、アクションが、接続ベースのアーキテクチャーで実装された *cloudant トリガー*・サービスと直接対話するようになっています。

`wsk trigger delete`、`wsk trigger update`、および `wsk trigger get` でも同様のフィード・アクション・プロトコルが発生します。    
