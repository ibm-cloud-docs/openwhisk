...
Liez un service à une action à l'aide de la commande `ibmcloud wsk service bind` fournie par le plug-in d'interface de ligne de commande [{{site.data.keyword.openwhisk_short}}](./bluemix_cli.html). D'autres informations sont fournies dans la section [Limitations](./binding_services.html#limitations). 
...
A partir du résultat, vous pouvez voir que l'ensemble de données d'identification approprié est lié à l'action. Là encore, vous pouvez exécuter la commande `ibmcloud wsk action get` pour en être certain. 
```
ibmcloud wsk action get hello parameters
```
{: pre}
...
Déconnectez un service d'une action à l'aide de la commande `ibmcloud wsk service unbind`. La commande `service unbind` retire les liaisons existantes créées par la commande `service bind`. 
